...
class Project extends CActiveRecord {
    ...
    /**
     * Buat Ngedapetin progress dari project di itung dari jumlah task yang selesai dan jumlahnya
     * @param Int $project_id project idny
     */
    public function  getProgress($project_id) {
        //ambil data semua project 
        $total_project = Yii::app()->db->createCommand()->select('COUNT(task_id) as total_task')->from('tasks')
                         ->where('task_project_id=:id', array(':id' => $project_id))->queryRow();
        $total_project_done = Yii::app()->db->createCommand()->select('COUNT(task_id) as total_task')->from('tasks')
                         ->where('task_project_id=:id AND task_status=:task_status', array(':id' => $project_id, ':task_status' => '1'))->queryRow();
        if(intval($total_project['total_task']) > 0 && intval($total_project_done['total_task']) > 0) {
            $persentase = intval($total_project_done['total_task']) /intval($total_project['total_task']) * 100;
